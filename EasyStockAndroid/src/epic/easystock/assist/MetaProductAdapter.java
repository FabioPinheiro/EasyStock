...
public class MetaProductAdapter extends ArrayAdapter<MetaProduct> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		//TextView labelViewDescription = (TextView) rowView.findViewById(R.id.product_row_label_Amount);

		// 4. Set the text for textView
		labelViewIdProduct.setText(productArrayList.get(position).getKey().getId().toString());
		labelViewName.setText(productArrayList.get(position).getProduct().getName());
		labelViewBarCode.setText(productArrayList.get(position).getProduct().getBarCode().toString());
		labelViewDescription.setText(productArrayList.get(position).getProduct().getDescription());
		//FIXME labelViewDescription.setText(productArrayList.get(position).getAmount());
