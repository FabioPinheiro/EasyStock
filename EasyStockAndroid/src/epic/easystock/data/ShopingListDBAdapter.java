 ...
 */
public class ShopingListDBAdapter {
	private final String LOG_TAG = this.getClass().getCanonicalName();
	private static final String DATABASE_NAME = "ShopingList_data";
	private static final int DATABASE_VERSION = 1;
	private static Map<Long, ShopingListDB> openShopingLists = new TreeMap<Long, ShopingListDB>();
	private Context mCtx;
	
	public ShopingListDBAdapter(Context context) {
		this.mCtx = context;
	}
	public void closeAllShopingLists(){
		for (ShopingListDB it : openShopingLists.values()) {
			it.close();
		}
	}
	public ShopingListDB getShopingListDB(Long ShopingListKey){//, String ShopingListName){
		ShopingListDB aux = openShopingLists.get(ShopingListKey);
		if (aux == null){
			aux = new ShopingListDB(ShopingListKey, EndPointCall.getShopingListNameFromKey(ShopingListKey));
			aux.open();
			openShopingLists.put(ShopingListKey,aux);
		}
		return aux;
	}
	
	//#***********************************************************************************#//
	public class ShopingListDB {
		protected final String DATABASE_CREATE;
		protected final String DATABASE_TABLE;
		protected boolean isOpen = false;
		protected DatabaseHelper mDbHelper;
		protected SQLiteDatabase mDb;
		private long ShopingListKey;

		
		public ShopingListDB(Long ShopingListKey, String ShopingListName) { //REMOVE ShopingListName
			//this.mCtx = context;
			this.ShopingListKey = ShopingListKey;
			this.DATABASE_TABLE = "table_" + ShopingListKey + "_" + ShopingListName ;
		 ...
		 */
		public ShopingListDB open() throws SQLException {
			if(!isOpen){
				//Log.d(LOG_TAG, EndPointCall.ShopingList_IS_ALREADY_OPEN);
		...
		public PantrySynchronizationDTO getPantrySynchronizationDTO() {
			...
			dto.setPantryKey(this.ShopingListKey);
